...
bool OpenFiles(Files *files_struct, char *argv[]) {
  ...
  if (files_struct->cfg == NULL || files_struct->memin == NULL ||
      files_struct->memout == NULL || files_struct->regout == NULL ||
      files_struct->traceinst == NULL || files_struct->tracedb == NULL) {
    return ERROR_CODE;
  } else {
    return SUCCESS;
  }
