...
bool EnterToExec(Station *res_station, Station *res_station_2, int res_size,
                 int res_size_2, int nr_of_exec_units,
                 int nr_of_working_exec_units, int cycle, int cycles_in_exec);
void RemoveLabel(Station *res_station, int type, int offset, int res_size,
                 float dst_result);
float DoArithmeticCalc(Station *inst);
void PutInReservationStation(Station *required_res_station,
                             int inst_queue_size);
int getReservationType(Station *res_station);
void DealWithCDB(CfgParameters *cfg_parameters, int cycle);
bool CDB(CfgParameters *cfg_parameters, Station *res_station, int size,
         int cycle);
...
void PrepareReservationStations(CfgParameters *cfg_parameters) {
  add_sub_res_stations =
      (Station *)calloc(cfg_parameters->add_nr_reservation, sizeof(Station));
  if (add_sub_res_stations == NULL) {
    exit(ERROR_CODE);
  }
  mul_res_stations =
      (Station *)calloc(cfg_parameters->mul_nr_reservation, sizeof(Station));
  if (mul_res_stations == NULL) {
    exit(ERROR_CODE);
  }
  divide_res_stations =
      (Station *)calloc(cfg_parameters->div_nr_reservation, sizeof(Station));
  if (divide_res_stations == NULL) {
    exit(ERROR_CODE);
  }
  load_res_stations =
      (Station *)calloc(cfg_parameters->mem_nr_load_buffers, sizeof(Station));
  if (load_res_stations == NULL) {
    exit(ERROR_CODE);
  }
  store_res_stations =
      (Station *)calloc(cfg_parameters->mem_nr_store_buffers, sizeof(Station));
...
//
// arguments:
// InstQueue inst - the inst we want to enter to the reservation station (it
// will be the inst_queue[0]). Station *res_station - the reservation station we
// want to enter the inst. int size - the slot we want to put the inst in the
// reservation station data structure. int cycle - current cycle. CfgParameters
// *cfg_parameters - the paramenters as in parameters.txt
//
...
// execute an inst. This function go through all the reservation satations and
// finds all the insts that are ready to exec, and sets their is_ready_for_exec
// flag to true. after that it calls the DealWithCDB that will send inst to the
// CDB (if available).
//
void Exec(CfgParameters *cfg_parameters, int cycle) {
  ...
  working_units = GetNumberOfWorkingExecUnits(
      add_sub_res_stations, cfg_parameters->add_nr_reservation);
  SetReadyForExec(add_sub_res_stations, cfg_parameters->add_nr_reservation,
                  cycle);
  EnterToExec(add_sub_res_stations, NULL, cfg_parameters->add_nr_reservation, 0,
              cfg_parameters->add_nr_units, working_units, cycle,
              cfg_parameters->add_delay);

  // MUL
  working_units = GetNumberOfWorkingExecUnits(
      mul_res_stations, cfg_parameters->mul_nr_reservation);
  SetReadyForExec(mul_res_stations, cfg_parameters->mul_nr_reservation, cycle);
  EnterToExec(mul_res_stations, NULL, cfg_parameters->mul_nr_reservation, 0,
              cfg_parameters->mul_nr_units, working_units, cycle,
              cfg_parameters->mul_delay);

  // DIV
  working_units = GetNumberOfWorkingExecUnits(
      divide_res_stations, cfg_parameters->div_nr_reservation);
  SetReadyForExec(divide_res_stations, cfg_parameters->div_nr_reservation,
                  cycle);
  EnterToExec(divide_res_stations, NULL, cfg_parameters->div_nr_reservation, 0,
              cfg_parameters->div_nr_units, working_units, cycle,
              cfg_parameters->div_delay);

  // LOAD or STORE
  working_units = GetNumberOfWorkingExecUnits(
      load_res_stations, cfg_parameters->mem_nr_load_buffers);
  mem_working_units = working_units + GetNumberOfWorkingExecUnits(
                                          store_res_stations,
                                          cfg_parameters->mem_nr_store_buffers);

  SetReadyForExec(load_res_stations, cfg_parameters->mem_nr_load_buffers,
                  cycle);
  SetReadyForExec(store_res_stations, cfg_parameters->mem_nr_store_buffers,
                  cycle);
  ...
  {
          working_units = GetNumberOfWorkingExecUnits(store_res_stations,
  cfg_parameters->mem_nr_store_buffers);
          //mem_working_units = working_units +
  GetNumberOfWorkingExecUnits(load_res_stations,
  cfg_parameters->mem_nr_load_buffers); SetReadyForExec(store_res_stations,
  cfg_parameters->mem_nr_store_buffers, cycle); EnterToExec(store_res_stations,
  cfg_parameters->mem_nr_store_buffers, STORE_NR_UNITS, 0, cycle,
  cfg_parameters->mem_delay);
  }*/
  // CDBs
...
void DealWithCDB(CfgParameters *cfg_parameters, int cycle) {
  ...
  CDB(cfg_parameters, add_sub_res_stations, cfg_parameters->add_nr_reservation,
      cycle);
  CDB(cfg_parameters, mul_res_stations, cfg_parameters->mul_nr_reservation,
      cycle);
  CDB(cfg_parameters, divide_res_stations, cfg_parameters->div_nr_reservation,
      cycle);
  is_load_in_CDB = CDB(cfg_parameters, load_res_stations,
                       cfg_parameters->mem_nr_load_buffers, cycle);
  // if (is_load_in_CDB == false) {
  CDB(cfg_parameters, store_res_stations, cfg_parameters->mem_nr_store_buffers,
      cycle);
  //}
}

// clears resorvation station slot to its defaults (0, NULL and false).
// this is done once the slot is needed to be removed from a resorvation
// station.
//
void ClearResSlot(Station *res_station, int offset) {
...
// sets the "is_ready_for_exec" bit inside Station struct once the instruction
// is ready to enter the execution unit
//
// arguments:
...
//
// the function returns the number of busy execution units for a specific
// reservation station.
//
int GetNumberOfWorkingExecUnits(Station *res_station, int res_size) {
  int i, counter = 0;
...
// finds the first inst that has the field 'is_ready_for_exec' equals true, and
// sets its field to 'is_in_exec' to true (only if an exec unit is available).
//
// arguments:
...
bool EnterToExec(Station *res_station, Station *res_station_2, int res_size,
  ...
  for (i = 0; i < res_size; i++) {
    if (res_station[i].is_ready_for_exec == true) {
      if (!(res_station[i].is_load == true ||
            res_station[i].is_store == true)) //
      {                                       //
      ...
      if (res_station[i].is_load == true || res_station[i].is_store == true) {
        ...
        if (is_first == true) {
          ...
          issue_list[res_station[i].PC].cycle_execute_end =
              res_station[i].cycle_to_finish_exec;
        } else {
...
bool CDB(CfgParameters *cfg_parameters, Station *res_station, int size,
  ...
  for (i = 0; i < size; i++) {
    if (cycle > res_station[i].cycle_to_finish_exec &&
        res_station[i].is_in_exec == true) {
      ...
      RemoveLabel(mul_res_stations, getReservationType(&res_station[i]), i,
                  cfg_parameters->mul_nr_reservation, dst_result);
      RemoveLabel(divide_res_stations, getReservationType(&res_station[i]), i,
                  cfg_parameters->div_nr_reservation, dst_result);
      RemoveLabel(load_res_stations, getReservationType(&res_station[i]), i,
                  cfg_parameters->mem_nr_load_buffers, dst_result);
      RemoveLabel(store_res_stations, getReservationType(&res_station[i]), i,
                  cfg_parameters->mem_nr_store_buffers, dst_result);

      issue_list[res_station[i].PC].cycle_write_cdb = cycle;

      PrintTo_tracecdb_file(files_struct.tracedb, res_station[i], i, cycle,
                            dst_result);
...
void RemoveLabel(Station *res_station, int type, int offset, int res_size,
  ...
  int i;
  for (i = 0; i < res_size; i++) {
    if (res_station[i].q_j == type &&
        res_station[i].q_j_station_offset == offset) {
      res_station[i].q_j = 0;
      res_station[i].q_j_station_offset = 0;
      res_station[i].v_j = dst_result;
    }
    if (res_station[i].q_k == type &&
        res_station[i].q_k_station_offset == offset) {
      res_station[i].q_k = 0;
...
int LastCDBCycle(CfgParameters *cfg_parameters) {
  ...
  max_values[0] = FindLastExecCycle(add_sub_res_stations,
                                    cfg_parameters->add_nr_reservation);
  max_values[1] =
      FindLastExecCycle(mul_res_stations, cfg_parameters->mul_nr_reservation);
  max_values[2] = FindLastExecCycle(divide_res_stations,
                                    cfg_parameters->div_nr_reservation);
  max_values[3] =
      FindLastExecCycle(load_res_stations, cfg_parameters->mem_nr_load_buffers);
  max_values[4] = FindLastExecCycle(store_res_stations,
                                    cfg_parameters->mem_nr_store_buffers);
...
bool isBusy(CfgParameters *cfg_parameters) {
  int total_busy = 0;
  total_busy =
      getNumOfBusy(add_sub_res_stations, cfg_parameters->add_nr_reservation);
  total_busy +=
      getNumOfBusy(mul_res_stations, cfg_parameters->mul_nr_reservation);
  total_busy +=
      getNumOfBusy(divide_res_stations, cfg_parameters->div_nr_reservation);
  total_busy +=
      getNumOfBusy(load_res_stations, cfg_parameters->mem_nr_load_buffers);
  total_busy +=
      getNumOfBusy(store_res_stations, cfg_parameters->mem_nr_store_buffers);
  if (total_busy > 0) {
    return true;
  }
  return false;
