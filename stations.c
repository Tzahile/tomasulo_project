...
bool LowestRelativePC(Station *res_station, int res_size, Station *res_station_2, int res_size_2, int cycle);
...
void Exec(CfgParameters *cfg_parameters, int cycle)
{
	...
	is_mem_just_entered = EnterToExec(load_res_stations, store_res_stations, cfg_parameters->mem_nr_load_buffers, cfg_parameters->mem_nr_store_buffers, LOAD_NR_UNITS, mem_working_units, cycle, cfg_parameters->mem_delay);
...
void SetReadyForExec(Station *res_station, int res_size, int cycle)
{
	...
	{
		is_memory_busy = (busy_memory[res_station[i].imm] != 0 && busy_memory[res_station[i].imm] < res_station[i].PC);
...
bool EnterToExec(Station *res_station, Station *res_station_2, int res_size, int res_size_2, int nr_of_exec_units, int nr_of_working_exec_units, int cycle, int cycles_in_exec)
{
	...
	for (i = 0; i < res_size; i++)
	{
		if (res_station[i].is_ready_for_exec == true)
		{
			...
			if (res_station[i].is_load == true || res_station[i].is_store == true)
			{
				is_first = LowestRelativePC(res_station, res_size, res_station_2, res_size_2, cycle);
...
bool LowestRelativePC(Station *res_station, int res_size, Station *res_station_2, int res_size_2, int cycle)
{
	...
	for (i = 0; i < res_size; i++)
	{
		if (res_station[i].is_ready_for_exec == true && res_station[i].is_in_exec == false)
		{
	...
	{
		if (res_station_2[i].is_ready_for_exec == true && res_station[i].is_in_exec == false)
		{
